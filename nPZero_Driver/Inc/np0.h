 ...
 * @file np0.h
 *
 * @brief Header file for nPZero Driver.
 *
 * This header file contains declarations and definitions for the nPZero driver for controlling and managing
 * the nPZero power-saving IC.
 * It provides function prototypes, constants, enumerations and structures necessary for interacting with the nPZero on
 * a register level.
 *
 * This driver is designed to work with the nPZero power-saving IC from Nanopower Semiconductor.
...
/** Reset Reason, see np0_register_sta1_s. */
typedef enum
{
...
typedef struct
{
    np0_resetsource_e reset_source : 3; /**< Indicates the reason for the device reset, see @np0_resetsource_e. */
...
typedef struct
{
    ...
    np0_host_power_mode_e pswh_mode : 2; /**< Sets the host power switch mode (SW_HP pin), see np0_host_power_mode_e. */
...
typedef struct
{
    np0_power_mode_e pwmod : 2;          /**< Controls peripheral power mode, see np0_power_mode_e. */
    np0_polling_mode_e tmod : 2;         /**< Controls peripheral polling mode, see np0_polling_mode_e. */
    np0_power_switch_mode_e pswmod : 2;  /**< Sets the peripheral power switch mode (pins SW_LP*),
                                          * see @np0_power_switch_mode_e. */
    np0_interrupt_pin_mode_e intmod : 2; /**< Sets the peripheral interrupt pin mode (pins INT*), either as an input
                                          * that can be used during polling, or as a trigger output,
                                          * see @np0_interrupt_pin_mode_e. */
} np0_register_cfgp_s;

/**  Peripheral Mode register for configuration of the different modes. */
typedef struct
{
    np0_comparison_mode_e cmod : 1; /**< Comparison mode of over/under thresholds.
    ...
    np0_data_type_e dtype : 2;      /**< Data type of value read From peripheral, see @np0_data_type_e. */
    ...
    uint8_t swprreg : 1;     /**< Swap high/low read registers of peripheral to match endianess of the data. (0: Little
                              * endian; 1: Big endian). */
    np0_spimod_e spimod : 2; /**< Sets the SPI mode if SPI communication is enabled, see @np0_spimod_e. */
} np0_register_modp_s;
...
typedef struct
{
	np0_register_sta1_s status1;
	np0_register_sta2_s status2;
} np0_status_s;
...
typedef struct
{
    ...
    uint8_t wakeup_enable : 1; /**< Enables wake up on internal ADC channel trigger or external ADC channel trigger.
                                * (0: ADC trigger disabled, 1: ADC trigger enabled). */
} np0_adc_config_channels_s;

/** Struct that holds all registers that can be configured for each peripheral. */
typedef struct
{
    /** Peripheral Address Configuration (ADDRP). */
    np0_com_protocol_e communication_protocol; /**< Sets which communication protocol should be
                                                * used with each peripheral.(0: I2C, 1: SPI). */

    /** Peripheral Configuration(CFGP). */
    np0_power_mode_e power_mode;                 /**< Controls peripheral power mode. */
    np0_polling_mode_e polling_mode;             /**< Controls peripheral polling mode. */
    np0_power_switch_mode_e power_switch_mode;   /**< Sets the peripheral power switch mode (pins SW_LP*). */
    np0_interrupt_pin_mode_e interrupt_pin_mode; /**< Sets the peripheral interrupt pin mode (pins INT*),
                                                  * either as an input that can be used during polling,
                                                  * or as a trigger output. */

    /** Peripheral Mode Configuration (MODP). */
    np0_comparison_mode_e comparison_mode;   /**< Comparison mode for threshold values. */
    np0_data_type_e sensor_data_type;               /**< Data type of value read From peripheral. */
    np0_multibyte_e multi_byte_transfer_enable  : 1; /**< Transmit sequential read and write addresses as multi-byte transfers.
                                              * (0:Disable multi-byte transfers; 1: Enable multi-byte transfers). */
    np0_endianess_e swap_registers : 1;  /**< Swap high/low read registers of peripheral to match endianess
    ...
    union
    {
        ...
        struct
        {
            ...
            np0_spimod_e mode;                    /**< Sets the SPI mode if SPI communication is enabled (MODP). */
        } spi_cfg;
    };
    ...
    np0_pre_wait_time_e pre_wait_time;   /**< Pre-initialization wait time. */
    np0_post_wait_time_e post_wait_time; /**< Post-initialization wait time. */

} np0_peripheral_config_s;

/** Struct that holds the configuration of the nP0 device. */
typedef struct
{
    ...
    np0_host_power_mode_e host_power_mode;     /**< Sets the host power switch mode (SW_HP pin). */
    ...
    uint8_t wake_up_per4 : 1; /**< Enables peripheral 4 as a wake up source when triggered. (0: Peripheral does not
                               * wake up system, 1: Peripheral can wake up system). */
    np0_wakeup_e wake_up_any_or_all : 1; /**< (wupmod) Controls if any wake up source trigger is sufficient to wake up
                                          * the system, or if all wake up source triggers are necessary to wake up.
                                          * (0: Wake up system on any trigger, 1: Wake up system on all triggers). */

    /** System Config 2 (SYSCFG2) for wakeup configuration. */
    np0_sclk_div_e system_clock_divider; /**< Controls system clock divider. */
    np0_sclk_sel_e system_clock_source;  /**< Controls system clock source between internal slow oscillator or
                                          * crystal oscillator. */

    /** System Config 3 (SYSCFG3) for pin, I2C pullup and clock out configuration. */
	np0_io_str_e 		io_strength;  	/**< IO pull-up strength configuration */
	np0_i2c_pull_sel_e 	i2c_pull_mode;	/**< IO pull-up mode */
	np0_spi_auto_e		spi_auto;		/**< SPI pin configuration */
    np0_xo_clkout_div_e xo_clock_out_sel;	/**< CLK_OUT pin configuration */
    ...
    np0_int_pin_pull_e interrupt_pin_pull_up_pin1; /**< Interrupt Config for interrupt pull-up resistors and
                                                    * pull-up strength for peripheral 1. */
    np0_int_pin_pull_e interrupt_pin_pull_up_pin2; /**< Interrupt Config for interrupt pull-up resistors and
                                                    * pull-up strength for peripheral 2. */
    np0_int_pin_pull_e interrupt_pin_pull_up_pin3; /**< Interrupt Config for interrupt pull-up resistors and
                                                    * pull-up strength for peripheral 3. */
    np0_int_pin_pull_e interrupt_pin_pull_up_pin4; /**< Interrupt Config for interrupt pull-up resistors and
                                                    * pull-up strength for peripheral 4. */

    /** Peripheral configurations (Disabled if null). */
    np0_peripheral_config_s *peripherals[4];
    ...
    np0_adc_clk_e adc_clock_sel;     /**< Controls ADC clock. */

    np0_adc_config_channels_s *adc_channels[2];
...
/**
 * @brief Reads the sleep_rst register and stores it in np0_register_sleep_rst_s struct.
 *
 *
...
/**
 * @brief Reads the ID register and stores it in np0_register_id_s struct.
 *
 *
...
/**
 * @brief Reads the sta1 register and writes it to np0_register_sta1_s struct.
 *
 *
...
/**
 * @brief Reads the sta2 register and writes it to np0_register_sta2_s struct.
 *
 *
...
/**
 * @brief Reads the pswctl register and writes it to np0_register_pswctl_s struct.
 *
 *
...
/**
 * @brief Reads the syscfg1 register and writes it to np0_register_syscfg1_s struct.
 *
 *
...
/**
 * @brief Reads the syscfg2 register and writes it to np0_register_syscfg2_s struct.
 *
 *
...
/**
 * @brief Reads the syscfg3 register and writes it to np0_register_syscfg2_s struct.
 *
 *
...
/**
 * @brief Reads the tout register and writes it to np0_register_tout_s struct.
 *
 *
...
/**
 * @brief Reads the intcfg register and writes it to np0_register_intcfg_s struct.
 *
 *
...
/**
 * @brief Reads the throva1 register and writes it to np0_register_throva1_s struct.
 *
 *
...
/**
 * @brief Reads the throva2 register and writes it to np0_register_throva2_s struct.
 *
 *
...
/**
 * @brief Reads the thruna1 register and writes it to np0_register_thruna1_s struct.
 *
 *
 *
...
/**
 * @brief Reads the thruna2 register and writes it to np0_register_thruna2_s struct.
 *
 *
...
/**
 * @brief Reads the last value from internal ADC channel (VBAT) and stores it in np0_register_adc_core_s struct.
 *
 *
...
/**
 * @brief Reads the last value from external ADC channel (ADC_IN) and stores it in np0_register_adc_ext_s
 * struct.
 *
 *
...
/**
 * @brief Reads one SRAM register and writes it to np0_register_sram_s struct.
 *
 *
...
/**
 * @brief Reads the cfgp register that is connected to the low power switch and writes it to np0_register_cfgp_s
 * struct.
 *
 *
...
/**
 * @brief Reads the modp register that is connected to the low power switch and writes it to np0_register_modp_s
 * struct.
 *
 *
 * @param [in] sw Low power switch indicates which peripheral will be read.
...
/**
 * @brief Reads the perp register that is connected to the low power switch and writes it to np0_register_perp_s
 * struct.
 *
 *
 * @param [in] sw Low power switch indicates which peripheral will be read.
...
/**
 * @brief Reads the twtp register that is connected to the low power switch and writes it to np0_register_twtp_s
 * struct.
 *
 *
 * @param [in] sw Low power switch indicates which peripheral will be read.
...
/**
 * @brief Reads the valp register that is connected to the low power switch and writes it to np0_register_valp_s
 * struct.
 *
 *
 * @param [in] sw Low power switch indicates which peripheral will be read.
