/**
 * @file np0_device_control.c
 * @brief Implementation of functions to read and handle status registers (STA1 and STA2), put nP0
...
#include "../Inc/np0_device_control.h"
#include <stdio.h>
...
typedef struct
{
    np0_register_throva1_s throva1; /**< Struct Internal ADC (VBAT)Threshold Over Value. */
    np0_register_thruna1_s thruna1; /**< Struct Internal ADC (VBAT)Threshold Under Value. */
} int_adc_channel_config_s;

/** Struct that holds External ADC configuration including threshold values. */
typedef struct
{
    np0_register_throva2_s throva2; /**< Struct External ADC (ADC_IN) Threshold Over Value. */
    np0_register_thruna2_s thruna2; /**< Struct External ADC (ADC_IN) Threshold Under Value. */
} ext_adc_channel_config_s;

/** Struct that holds all registers that can be configured for each peripheral. */
typedef struct
{
    np0_register_cfgp_s cfgp;   /**< Struct Peripheral Config register. */
    np0_register_modp_s modp;   /**< Struct Peripheral Mode register. */
    np0_register_perp_s perp;   /**< Struct Peripheral Polling register. */
    np0_register_ncmdp_s ncmdp; /**< Struct Peripheral Number of Commands register. */
    np0_register_sram_s sram_p;
    np0_register_addrp_s addrp;   /**< Struct Peripheral Address register. */
    np0_register_rregp_s rregp;   /**< Struct Peripheral Address of I2C register. */
    np0_register_throvp_s throvp; /**< Struct Peripheral Threshold Over Value. */
    np0_register_thrunp_s thrunp; /**< Struct Peripheral Threshold Under Value. */
    np0_register_twtp_s twtp;     /**< Peripheral Wait Time before reading value. */
    np0_register_tcfgp_s tcfgp;   /**< Peripheral Wait Time configuration. */
} peripheral_config_s;
 ...
 */
static bool set_global_timeout(np0_device_config_s * device_config)
{
    np0_register_tout_s time_out_config = {0};
    ...
    if (device_config->global_timeout > 0)
    {
        ...
        if (np0_write_TOUT(time_out_config) != OK)
        {
    ...
    {
        printf("Invalid invalid global timeout\r\n");
        return false;
    }
...
static bool set_power_switch_control(np0_device_config_s * device_config)
{
    np0_register_pswctl_s pswctl = {0};
...
static bool set_system_config1(np0_device_config_s * device_config)
{
    np0_register_syscfg1_s syscfg1 = {0};
    ...
    if (np0_write_SYSCFG1(syscfg1) != OK)
    {
...
static bool set_system_config2(np0_device_config_s * device_config)
{
    np0_register_syscfg2_s syscfg2 = {0};
    ...
    if (np0_write_SYSCFG2(syscfg2) != OK)
    {
...
static bool set_system_config3(np0_device_config_s * device_config)
{
    np0_register_syscfg3_s syscfg3 = {0};
    ...
    if (np0_write_SYSCFG3(syscfg3) != OK)
    {
...
static bool set_interrupt_pin_config(np0_device_config_s * device_config)
{
    np0_register_intcfg_s intcfg = {0};
...
static bool set_peripheral_power_mode(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (np0_write_CFGP(switch_id, peripheral[index].cfgp) != OK)
    {
...
static bool set_peripheral_mode(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (np0_write_MODP(switch_id, peripheral[index].modp) != OK)
    {
...
static bool set_peripheral_polling_period(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (np0_write_PERP(switch_id, peripheral[index].perp) != OK)
    {
...
static bool set_peripheral_init_cmds_number(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (device_config->peripherals[index]->communication_protocol == COM_I2C)
    {
        ...
        if (device_config->peripherals[index]->i2c_cfg.command_num > 0)
        {
            if (device_config->peripherals[index]->multi_byte_transfer_enable ==
                MULTIBYTE_TRANSFER_ENABLE)
            {
                SRAM_number_bytes_2_write =
                    device_config->peripherals[index]->i2c_cfg.command_num * 3; // ADD;val_h;val_l
            }
            else
            {
                SRAM_number_bytes_2_write =
                    device_config->peripherals[index]->i2c_cfg.command_num * 2; // ADD;val_l
            }

            for (uint8_t i = 0; i < SRAM_number_bytes_2_write; i++)
            {
                if (np0_write_SRAM(SRAM_START + m_sram_count,
...
//                        SRAM_START + m_sram_count);
        ...
        {
            printf("NO SRAM Space available for peripheral %d", index + 1);
        }
    }
    else if (device_config->peripherals[index]->communication_protocol == COM_SPI)
    {
    	peripheral[index].ncmdp.ncmdp = device_config->peripherals[index]->spi_cfg.bytes_from_sram_num;
        if (device_config->peripherals[index]->spi_cfg.bytes_from_sram_num > 0)
        {
            if (device_config->peripherals[index]->multi_byte_transfer_enable ==
                MULTIBYTE_TRANSFER_ENABLE)
            {
                SRAM_number_bytes_2_write =
                    device_config->peripherals[index]->spi_cfg.bytes_from_sram_num * 2; // ADD;val_h;val_l
            }
            else
            {
                SRAM_number_bytes_2_write =
                    device_config->peripherals[index]->spi_cfg.bytes_from_sram_num; // ADD;val_l
            }
...
//                         SRAM_START + m_sram_count);
    ...
    if (np0_write_NCMDP(switch_id, peripheral[index].ncmdp) != OK)
    {
...
static bool set_peripheral_address(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    if (device_config->peripherals[index]->communication_protocol == COM_I2C)
    {
    ...
    if (np0_write_ADDRP(switch_id, peripheral[index].addrp) != OK)
    {
...
static bool set_peripheral_i2c_read_register(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
        ...
        device_config->peripherals[index]->communication_protocol == COM_I2C)
    {
        ...
        if (np0_write_RREGP(switch_id, peripheral[index].rregp) != OK)
        {
...
static bool set_peripheral_under_threshold(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (np0_write_THRUNP(switch_id, peripheral[index].thrunp) != OK)
    {
...
static bool set_peripheral_over_threshold(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (np0_write_THROVP(switch_id, peripheral[index].throvp) != OK)
    {
...
static bool set_peripheral_time_to_wait_config(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    ...
    if (np0_write_TCFGP(switch_id, peripheral[index].tcfgp) != OK)
    {
...
static bool set_peripheral_time_to_wait(np0_device_config_s * device_config,
    peripheral_config_s * peripheral, int index, np0_psw_e switch_id)
{
    peripheral[index].twtp.twtp = device_config->peripherals[index]->time_to_wait;
    if (np0_write_TWTP(switch_id, peripheral[index].twtp) != OK)
    {
...
static bool validate_peripherals(np0_device_config_s * device_config)
{
...
static bool configure_peripherals(np0_device_config_s * device_config)
{
    peripheral_config_s peripherals[4] = {0};
    np0_psw_e switches[4] = {
...
static bool configure_internal_adc(np0_device_config_s * device_config)
{
        ...
        device_config->adc_channels[0]->under_threshold > 0)
    {
        ...
        if (np0_write_THROVA1(adc_channel.throva1) != OK)
        {
        ...
        if (np0_write_THRUNA1(adc_channel.thruna1) != OK)
        {
...
static bool configure_external_adc(np0_device_config_s * device_config)
{
        ...
        device_config->adc_channels[1]->under_threshold > 0)
    {
        ...
        if (np0_write_THROVA2(adc_channel.throva2) != OK)
        {
        ...
        if (np0_write_THRUNA2(adc_channel.thruna2) != OK)
        {
...
static bool configure_global_settings(np0_device_config_s * device_config)
{
    // Set global time out
...
{
    np0_register_adc_ext_s get_adc_ext_val = {0};
    np0_register_syscfg1_s syscfg1 = {0};
    np0_register_syscfg2_s syscfg2 = {0};

    if (np0_read_SYSCFG1(&syscfg1) != OK)
    {
    ...
    if (np0_read_SYSCFG2(&syscfg2) != OK)
    {
    ...
    {
        if (np0_read_ADC_EXT(&get_adc_ext_val) != OK)
        {
...
{
    np0_register_adc_core_s get_adc_core_val = {0};

    printf("Internal ADC channel (connected to VBAT) was triggered\r\n");
    if (np0_read_ADC_CORE(&get_adc_core_val) != OK)
    {
...
{
    np0_register_cfgp_s cfgp = {0};
    np0_register_valp_s valp = {0};
    np0_register_addrp_s addrp = {0};

    printf("External Trigger from Peripheral %d\r\n", psw_lp);

    if (np0_read_CFGP(psw_lp, &cfgp) != OK)
    {
    ...
    {
        if (np0_read_VALP(psw_lp, &valp) != OK)
        {
            printf("Failed to read VALP register for peripheral %d\r\n", psw_lp);
            return false;
        }

        if (np0_read_ADDRP(psw_lp, &addrp) != OK)
        {
...
/**
 * @brief Put nP0 Device in Sleep mode.
 */
void np0_device_go_to_sleep(void)
{
    printf("Enter sleep, disable I2C bus and transfer control to the nPZero\r\n");

    uint8_t sleep_rst_value = 0xFF;
    if (np0_write_SLEEP_RST(sleep_rst_value) != OK)
    {
...
/**
 * @brief Reset nP0 Device by software.
 */
