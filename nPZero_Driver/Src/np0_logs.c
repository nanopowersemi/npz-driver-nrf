...
static bool read_peripherals(np0_device_config_s *device_config)
{
    np0_psw_e switches[4] = {PSW_LP1, PSW_LP2, PSW_LP3, PSW_LP4}; // Corresponding switches

    // Iterate through the peripherals
    for (int j = 0; j < m_configured_count; j++)
    {
        np0_register_cfgp_s cfgp = {0};
        np0_register_modp_s modp = {0};
        np0_register_perp_s perp = {0};
        np0_register_ncmdp_s ncmdp = {0};
        np0_register_twtp_s twtp = {0};
        np0_register_tcfgp_s tcfgp = {0};
        ...
        if (np0_read_CFGP(switches[i], &cfgp) == OK)
        {
        ...
        if (np0_read_MODP(switches[i], &modp) == OK)
        {
        ...
        if (np0_read_PERP(switches[i], &perp) == OK)
        {
        ...
        if (np0_read_NCMDP(switches[i], &ncmdp) == OK)
        {
            ...
            (device_config->peripherals[i]->polling_mode == POLLING_MODE_PERIODIC_WAIT_INTERRUPT))
        {
            ...
            if (np0_read_ADDRP(switches[i], &addrp) == OK)
            {
        ...
        if ((device_config->peripherals[i]->polling_mode == POLLING_MODE_PERIODIC_READ_COMPARE_THRESHOLD) ||
            (device_config->peripherals[i]->polling_mode == POLLING_MODE_PERIODIC_WAIT_INTERRUPT_COMPARE_THRESHOLD))
        {
            np0_register_rregp_s rregp = {0};
            np0_register_thrunp_s thrunp = {0};
            np0_register_throvp_s throvp = {0};

            // Read and log RREGP register
            if (np0_read_RREGP(switches[i], &rregp) == OK)
            {
            ...
            if (np0_read_THRUNP(switches[i], &thrunp) == OK)
            {
            ...
            if (np0_read_THROVP(switches[i], &throvp) == OK)
            {
        ...
        if (np0_read_TWTP(switches[i], &twtp) == OK)
        {
        ...
        if (np0_read_TCFGP(switches[i], &tcfgp) == OK)
        {
...
static bool adc_config_read(np0_device_config_s *device_config)
{
    if (device_config->adc_channels[0]->wakeup_enable == 1)
    {
        ...
        np0_register_throva1_s throva1 = {0};
        np0_register_thruna1_s thruna1 = {0};

        if (np0_read_THROVA1(&throva1) != OK)
        {
        ...
        if (np0_read_THRUNA1(&thruna1) != OK)
        {
    ...
    if (device_config->adc_channels[1]->wakeup_enable == 1 && device_config->adc_ext_sampling_enable == 1)
    {
        ...
        np0_register_throva2_s throva2 = {0};
        np0_register_thruna2_s thruna2 = {0};

        if (np0_read_THROVA2(&throva2) != OK)
        {
        ...
        if (np0_read_THRUNA2(&thruna2) != OK)
        {
...
static bool peripheral_config_read(np0_device_config_s *device_config)
{
...
static bool global_config_read()
{
    ...
    char binary_str[9];  // Buffer to hold binary strings for each register value
    np0_status_e status; // Variable to hold the status of I2C read
    ...
    for (size_t i = 0; i < sizeof(global_register_info) / sizeof(global_register_info[0]); i++)
    {
        ...
        status = np0_read_register(global_register_info[i].reg_address, &data, 1);
        if (status != OK)
        {
...
{
    np0_register_sta1_s status1 = {0}; // Variable to store status1 register values
    np0_register_sta2_s status2 = {0}; // Variable to store status2 register values

    // Read the first status register
    if (np0_read_STA1(&status1) != OK)
    {
        return false;
    }

    // Read the second status register
    if (np0_read_STA2(&status2) != OK)
    {
        return false;
    }
